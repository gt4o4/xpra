...
typedef struct {
    ...
    int      us_submit;
    int      us_sync;
    /* every decoded frame is a zero-copy GPU frame - it never leaves
     * the GPU; these are the GL_NV_vdpau_interop handles.  The
     * surface pool slot stays pinned until
 ...
 * frame comes back from libva_decoder_decode as a GPU frame (the
 * vdp_* fields above); there is no CPU readback at all.
 *
